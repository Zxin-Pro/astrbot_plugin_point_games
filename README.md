...
- 🌐 **浏览器限定模式**：一键收拢玩法到网页端，群内指令全部转为引导（开关控制）
...
| `每日收税` | 凌晨0点自动收取余额0.1%税款（余额≥1000才扣，自动执行，税款入系统钱包） |

### 🏛 系统钱包

| 指令 | 说明 |
|---|---|
| `/系统` | 查看系统钱包（中央资金池）余额与累计流入/流出 |
| `/系统 存 [积分]` | 管理员：从自己普通余额存入系统钱包 |
| `/系统 取 [积分]` | 管理员：从系统钱包取出到个人余额 |
| `/系统 流水 [页数]` | 管理员：分页查看钱包流水（每页50条） |

> 资金池来源：所有玩家支出流水、转账手续费、每日税款、银行利息同额注入；
> 支出：钓鱼奖池抽成、每日红包（可配置）。余额不足时红包自动取消并全群通知。
...
| `/玩法模式 白名单\|黑名单` | 全局模式切换 |
| `/本群状态` | 查看本群与全局状态 |
| `/帮助` | 玩法介绍与指令列表（内置烛之游风格总览图，可在配置页 `help_image` 自定义图片路径） |
...
> 用途：把玩法全部收拢到网页端，群里只做引导。
> 开启后，聊天平台发送的任何本插件指令（转盘/签到/钓鱼/挖矿/修仙/抽卡/银行…全部 119 个处理器）一律失效，
> 统一回复：
> ```
> 🌐 本群积分游戏已切换为「仅限浏览器游玩」模式喵~
> 请到浏览器上完成相应的玩法：
> https://xn--15t839a.xyz
> ```
> - **网页端玩法完全不受影响**（网页桥接直接调用插件本体逻辑，不经过聊天指令管线），数值、冷却、概率与群内完全一致
> - 闲聊、其他插件的指令、LLM 对话均不受影响，只有命中本插件指令的消息会被拦
> - 开关是热生效的：改完配置重载插件即可，无需重启 AstrBot
> - 开关关闭后（默认）一切照旧，群内正常游玩
...
| `browser_only_mode` | bool | `false` | **浏览器限定游玩模式**：开启后聊天平台的任何积分游戏指令一律失效，统一回复引导文案与网站地址，仅网页端可玩（网页玩法不受影响） |
...
| 配置项 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `enable_wallet` | bool | `true` | 启用系统钱包（关闭后 /系统 不可用，资金池记账仍生效） |
| `fishing_reward_chance` | float | `20` | 钓鱼奖池抽成触发概率（%），0=关闭 |
| `fishing_reward_min` | float | `0.01` | 奖池抽取比例下限（%），0.01=万分之一 |
| `fishing_reward_max` | float | `10` | 奖池抽取比例上限（%） |
| `redpacket_from_wallet` | bool | `true` | 每日红包从系统钱包支出，余额不足取消并通知 |
